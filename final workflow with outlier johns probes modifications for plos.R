...
pdf("FigS5A.pdf", width=8, height=7, useDingbats=FALSE)
...
pdf("FigS5B.pdf", width=8, height=7, useDingbats=FALSE)
...
write.table(bottom_lclnames, "C:/Users/a a/Documents/Lab/Variation Recovery/hgnc_lcl_names_postreview.txt", sep="\t", row.names=F, col.names=F, quote=F)
write.table(bottom_stemnames, "C:/Users/a a/Documents/Lab/Variation Recovery/hgnc_stem_names_postreview.txt", sep="\t", row.names=F, col.names=F, quote=F)
write.table(names_all, "C:/Users/a a/Documents/Lab/Variation Recovery/hgnc_all_names_postreview.txt", sep="\t", row.names=F, col.names=F, quote=F)
...
#With all data points
pdf("FigS8C.pdf", width=7, height=7, useDingbats=FALSE)
ggplot(var_rat_all, aes(x=var, fill=Cell_type)) + geom_density(alpha=0.5) +xlab("Variance Between/Variance Within") + geom_vline(xintercept=cutoff_avg, linetype="dotted") + theme(panel.border=element_rect(color="black", fill=NA), panel.grid.major = element_blank(), panel.grid.minor= element_blank(),legend.position=leg_pos, panel.background=element_rect(fill='white'),legend.title=element_blank(), axis.title=element_text(size=18, face="plain"), axis.title.y=element_text(vjust=1.2), axis.title.x = element_text(vjust=-.35), panel.background=element_rect(fill='white')) + theme(text = element_text(size=18, face="bold")) +annotate(geom = "text", label=paste("p-value < 2.2 e-16 "), x=75, y=1.5) + scale_fill_manual(values=cols, labels=c("LCLs", "iPSCs"))
dev.off()
num_removed_varrat <- length(which(var_rat_all[,1]>8))/length(var_rat_all[,1])

#Var
#Variance in genes with eQTLs vs without
pdf("FigS10.pdf", width=8, height=7, useDingbats=FALSE)
leg_pos <- c(.07,.9)
...
pdf("FigS4AA.pdf", width=8, height=7, useDingbats=FALSE)
...
pdf("FigS4BA.pdf", width=8, height=7, useDingbats=FALSE)
...
for(i in 2:4) {
  plot(sum.PC$rotation[,1], sum.PC$rotation[,i],cex=2, col=color,pch=20,main=title.PC, cex.lab=1.5, ylim=c(-0.8,0.3), cex.axis=1.2, xlab=paste("PC 1 -", (sumsum$importance[2,1]*100),"% of variance", sep=" "), ylab=paste("PC",i,"-",(sumsum$importance[2,i]*100),"% of variance", sep=" "))
  text(sum.PC$rotation[,1], sum.PC$rotation[,i],labels=indiv.fs, cex = 1.25, pos=3)   
}  

pdf("FigS4AB.pdf", width=7, height=7, useDingbats=FALSE)
plot(sum.PC$rotation[,1], sum.PC$rotation[,2],cex=2, col=color,pch=20,main=title.PC, cex.lab=1.5, ylim=c(-0.8,0.3), cex.axis=1.2, xlab=paste("PC 1 -", (sumsum$importance[2,1]*100),"% of variance", sep=" "), ylab=paste("PC 2 -",(sumsum$importance[2,2]*100),"% of variance", sep=" "))
...
pdf("FigS4BB.pdf", width=7, height=7, useDingbats=FALSE)
plot(sum.PC$rotation[,1], sum.PC$rotation[,2],cex=2, col=color,pch=20,main=title.PC, cex.lab=1.5, cex.axis=1.2, ylim=c(-0.6,0.5), xlab=paste("PC 1 -", (sumsum$importance[2,1]*100),"% of variance", sep=" "), ylab=paste("PC 2 -",(sumsum$importance[2,2]*100),"% of variance", sep=" "))
text(sum.PC$rotation[,1], sum.PC$rotation[,2],labels=indiv.fl, cex = 1.25, pos=3)   
dev.off()
...
groupn <- c(rep("1", times=(length(cor_within_s)+length(cor_within_l))), rep("2", times=length(cor_all_stem)+length(cor_all_lcl)))
cor_df <- data.frame(cor_total_vec, type, group, groupn)
pdf("FigS6.pdf", width=9, height=7, useDingbats=FALSE)
ggplot(cor_df, aes(x=groupn, y=cor_total_vec)) + geom_boxplot(aes(fill=type), xlab=FALSE) + theme(text = element_text(size=18)) + annotate(geom = "text", label=paste("**p =", pv_w), x=1, y=0.98) + annotate(geom = "text", label=paste("**p =", pv_b), x=2, y=.97) + scale_x_discrete(labels=c("Within Individuals", "Between Individuals")) + theme(axis.title.x=element_blank(), panel.background=element_rect(fill='white')) + ylab("Pearson Correlation") + theme(axis.title.y = element_text(size=12, vjust=2.0), legend.title=element_blank()) #stat_boxplot(geom ='errorbar', aes(x=group))  
dev.off()

#boxplot of expression means for highly variable genes in ipscs in both cell types
pdf("FigS11.pdf", width=7, height=7, useDingbats=FALSE)
boxplot(expr_LCL_hs_mean, expr_LCL_mean, expr_stem_hs_mean, expr_stem_mean, names=c("LCL: high iPSC var", "LCL: all", "iPSC: high iPSC var", "iPSC: all"), ylab="Gene Expression")
dev.off()
t.test(expr_stem_hs_mean, expr_LCL_hs_mean)
t.test(expr_LCL_mean, expr_stem_mean)
